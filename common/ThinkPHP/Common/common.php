...
function import($class, $baseUrl = '', $ext='.class.php')
{
    ...
    if (empty($baseUrl))
	{
        $appPath = defined('BASE_APP_PATH')? BASE_APP_PATH : APP_PATH;
        if ('@' == $class_strut[0] || APP_NAME == $class_strut[0])
		{
            //加载当前项目应用类库
            $baseUrl = dirname($appPath);
            $class = substr_replace($class, basename($appPath).'/', 0, strlen($class_strut[0]) + 1);
        }
		...
		{ 	// 加载其他项目应用类库
            ...
            $baseUrl = PRO_PATH . '../' . $class_strut[0] . '/'.basename($appPath).'/';
        }
    }
    if (substr($baseUrl, -1) != '/')
        $baseUrl .= '/';
 ...
 * @param string $layer 模型类名后缀（同时也为模型类所在文件夹的名称）
 * @return Model
 */
function D($name = '', $layer = '')
{
	...
	if (strpos($name, '://'))
	{	// 指定项目
        $name = str_replace('://', '/'.strtolower($layer).'/', $name);
    }
	else
	{
        $name = C('DEFAULT_APP').'/'.strtolower($layer).'/'.$name;
    }
 ...
 * @param string $layer 	控制器类名后缀（同时也为控制器类所在文件夹的名称）
...
function A($name, $layer = '', $common = false)
{
    ...
    if ($common)
	{ 	// 独立分组情况下 加载公共目录类库
        import(str_replace('@/', '', $name).$layer, APP_PATH);
    }
	else
	{
        import($name.$layer);
...
// 生成目录安全文件
function build_dir_secure($dirs = '', $secureFilename = 'index.html', $secureContent = 'Deney Access!')
{
	$content = $secureContent;
	$files = explode(',', $secureFilename);
