...
class App {
    ...
    static public function init()
	{
		...
		{
            define('THEME_PATH', BASE_APP_PATH.basename(TMPL_PATH).'/'.(THEME_NAME ? THEME_NAME.'/' : ''));
            // define('APP_TMPL_PATH', __ROOT__.'/'.APP_NAME.(APP_NAME ? '/' : '').C('APP_GROUP_PATH').'/'.$group.basename(TMPL_PATH).'/'.(THEME_NAME ? THEME_NAME.'/' : ''));
            define('APP_TMPL_PATH', __ROOT__.'/'.APP_NAME.(APP_NAME ? '/' : '').C('APP_GROUP_PATH').'/'.$group.basename(dirname(TMPL_PATH)).'/'.basename(TMPL_PATH).'/'.(THEME_NAME ? THEME_NAME.'/' : ''));
        }
		else
		{
            ...
            define('APP_TMPL_PATH',__ROOT__.'/'.APP_NAME.(APP_NAME ? '/' : '').basename(dirname(TMPL_PATH)).'/'.basename(TMPL_PATH).'/'.$group.(THEME_NAME?THEME_NAME.'/':''));
        }
		
		/* 静态资源目录常量 */
		define('PUBLIC_RESOURCE', __SELF__.'resource/public/');							// 前后台公用的资源文件
		define('COMMON_RESOURCE', __SELF__.'resource/'.$group.'public/');				// 当前项目的所有主题公用的资源文件
		define('THEME_RESOURCE', __SELF__.'resource/'.$group.(THEME_NAME ? THEME_NAME.'/' : ''));	// 当前项目的当前主题的资源文件

		/* 用户资源目录常量 */
		define('USERFILES', __SELF__.'userfiles/');					// 用户文件根目录
		define('USERFILES_AVATARS', __SELF__.'userfiles/avatars/');			// 用户头像文件根目录
		define('USERFILES_PRODUCTS', __SELF__.'userfiles/products/');			// 用户商品文件根目录
